...
medians = {}
for x in range(0,len(country_list)):
    medians[country_list[x]] = np.median(df.xs(country_list[x])["daily_vaccinations"])
medians = dict(sorted(medians.items(),key= lambda x:x[1], reverse = True))
print(list(medians.keys())[0:3])
